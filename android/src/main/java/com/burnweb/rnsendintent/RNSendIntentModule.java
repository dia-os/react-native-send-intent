...
public class RNSendIntentModule extends ReactContextBaseJavaModule {
    ...
    private GeneralDdi gddi = new GeneralDdi();
    private boolean running_flag = false;
    private boolean mul_scan = false;
    private boolean isAuto = false;
    private boolean isAging = false;
    Thread ScannalThread;

    final byte DEEP_SLEEP[] = {0x7E, 0x01, 0x30, 0x30, 0x30, 0x30, 0x23, 0x50, 0x57, 0x52, 0x44, 0x53, 0x50, 0x3B, 0x03};
    final byte DEEP_SLEEP_response[] = {0x02, 0x01, 0x30, 0x30, 0x30, 0x30, 0x23, 0x50, 0x57, 0x52, 0x44, 0x53, 0x50, 0x15, 0x3b, 0x03};

    final byte SCNTRG1[] = {0x7E, 0x01, 0x30, 0x30, 0x30, 0x30, 0x23, 0x53, 0x43, 0x4E, 0x54, 0x52, 0x47, 0x31, 0x3B, 0x03};
    final byte SCNTRG1_response[] = {0x02, 0x01, 0x30, 0x30, 0x30, 0x30, 0x23, 0x53, 0x43, 0x4e, 0x54, 0x52, 0x47, 0x31, 0x06, 0x3b, 0x03};
    ...
    @ReactMethod
    public void openScanner(String dataUri, final Promise promise) {
            running_flag = true;
            ...
            StrComAttr ComAttr = new StrComAttr();
            ComAttr.setBaud(9600);
            ComAttr.setDatabits(8);
            ComAttr.setParity(0);
            ComAttr.setStopbits(1);
    
            int ret = -1;
    
            while ((ret = ddi.ddi_com_open(1, ComAttr)) < 0) {
                Log.i(TAG, "ddi_com_open err times:" + times + " ret:" + ret);
                times++;
                try {
                    Thread.sleep(50);
            ...
            if (ret < 0) {
                Log.e(TAG, "lrf_log,ddi_com_open fail ret:" + ret);
                return;
            }
            ...
            while (running_flag) {
                ...
                times++;
                if (times > 60) {
                    Log.e(TAG, "scanner read response,ret:" + ret + " times:" + times);
                    break;
                }
                if (flag == false) {
                    if (times == 1 || times == 5 || times == 9 || times == 13 || times == 17 || times == 21 || times == 25 || times == 29 || times == 33 || times == 37) {
                        ret = ddi.ddi_com_write(1, SCNTRG1, SCNTRG1.length);
                        Log.i(TAG, "scanner :SCNTRG1 write ret:" + ret + " times:" + times);
                    }
                    try {
                        Thread.sleep(50);
                    } catch (InterruptedException e) {
                    ...
                    ret = ddi.ddi_com_read(1, response, 17);
                    Log.i(TAG, "scanner read response:" + ret + "  " + byteArrayToHexString(response, ret));
                    if (ret == 17) {
                        if (Arrays.equals(response, SCNTRG1_response)) {
                            flag = true;
                            //Log.i(TAG, "scanner read response1:"+ret+"  "+byteArrayToHexString(response,ret));
                            Log.i(TAG, "lrf_log,set flag:" + flag);
                        } else {
                            continue;
                        }
                    } else {
                ...
                if (flag) {
                    ...
                    tmpData = new byte[2048];
                    int offset = 0;
                    //liuruifeng
                    while (offset < tmpData.length) {
                        ret = ddi.ddi_com_read(1, data, 1024);
                        Log.i(TAG, "lrf_log, ret:" + ret);
                        if (ret <= 0) {
                            break;
                        }
    
                        System.arraycopy(data, 0, tmpData, offset, ret);
                        offset += ret;
                        try {
                            Thread.sleep(70);
                        } catch (InterruptedException e) {
                        ...
                        Log.i(TAG, "lrf_log,offset:" + offset + " ret:" + ret);
                    }
                    Log.i(TAG, "scanner read data:" + offset + "  " + byteArrayToHexString(tmpData, offset));
                    if (offset > 0) {
                        byte[] qrcodeTmpData = new byte[offset];
                        System.arraycopy(tmpData, 0, qrcodeTmpData, 0, offset);
                        String str = new String(qrcodeTmpData);
                        mPromise = promise;
                        mPromise.resolve(str);
                        ddi.ddi_sys_set_beep(200);
                        break;
                    }
                }
            }
